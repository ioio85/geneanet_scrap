...
def get_dataframe_arbre_by_url(url: str):
    ...
    df = df[0]
    df['generation'] = df['Sosa'].apply(lambda x: 1 if 'Generation' in x else 0)
...
def get_lastname_with_sosa(chaine_name_surname: str = '', sosa: int = None,field:bool=False) -> str:
    if sosa:
        if not field:
            chaine_name_surname = df[df['Sosa'] == sosa]['Individual'].values[0]
...
def get_job_with_sosa(sosa: int = None) -> str:
    try:
        job = df[df['Sosa'] == sosa].to_dict('records')[0]['Occupations']
...
def get_birth_information_with_sosa(sosa: int = None, typeInfos: str = '') -> str:
    try:
        transco = {'date': 'Date of birth', 'address': 'Where born'}
...
def get_death_information_with_sosa(sosa: int = None, typeInfos: str = '') -> str:
    try:
        transco = {'date': 'Date of death', 'address': 'Where dead'}
        result = df[df['Sosa'] == sosa][list(transco.values())].fillna('').to_dict('records')[0]
        return result[transco[typeInfos]]
    except Exception as e:
        return False
...
def get_union_information_with_sosa(sosa: int = None, typeInfos: str = '') -> str:
    try:
        transco = {'date': "Date of marriage", 'address': "Where married"}
        result = df[df['Sosa'] == sosa][list(transco.values())].fillna('').to_dict('records')[0]
        return result[transco[typeInfos]]
    except Exception as e:
        return False
...
class Person:
    ...
    def get_fiche(self):
        ...
        if test_person_exist(self.sosa):
            ...
            return "la Individual n existe pas"
...
url = "https://gw.geneanet.org/epage1?birth=on&birth_place=on&child=on&color=&death=on&death_age=on&death_place=on&gen=on&lang=en&m=A&marr=on&marr_date=on&marr_place=on&n=pitavy&nz=page&oc=9&occu=on&ocz=0&p=jean&pz=etienne+armand&repeat=on&sosab=10&t=Z&v=7"
...
# df[df['Individual'] == "François DALBIN"]
# df[df['Sosa'] == 30060]
